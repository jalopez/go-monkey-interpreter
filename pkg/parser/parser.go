...
// Errors returns the errors while parsing
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	if function, ok := stmt.Value.(*ast.FunctionLiteral); ok {
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	expression := parsePrefixFn()

	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		parseInfixFn := p.infixParseFns[p.peekToken.Type]
		if parseInfixFn == nil {
			return expression
		}
		...
		expression = parseInfixFn(expression)
	}
...
func (p *Parser) parseExpressionList(rightToken token.Type) []ast.Expression {
	expressionList := []ast.Expression{}

	if p.peekTokenIs(rightToken) {
		p.nextToken()
...
func (p *Parser) appendError(erroredToken token.Token, msg string) {
	msg = fmt.Sprintf("%s (on line %d, col %d)", msg, erroredToken.Line, erroredToken.Column)
